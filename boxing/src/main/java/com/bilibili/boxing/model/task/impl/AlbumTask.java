...
public class AlbumTask {
    ...
    public AlbumTask() {
        ...
        this.mVideoAlbum=new AlbumEntity();
        this.mVideoAlbum.mBucketName="所有视频";
        //========yqy changed 2017-09-29===================
    }
    ...
    private void getAllVideoAlbum(ContentResolver cr){
        String[] videoColumn = new String[]{MediaStore.Video.Media._ID, MediaStore.Video.Media.DATA};
        Cursor cursorVideo= cr.query(MediaStore.Video.Media.EXTERNAL_CONTENT_URI, videoColumn, null, null,
                MediaStore.Video.Media.DATE_ADDED);
        try {
            if (cursorVideo != null && cursorVideo.moveToFirst()) {
                String picPath = cursorVideo.getString(cursorVideo.getColumnIndex(Media.DATA));
                String id = cursorVideo.getString(cursorVideo.getColumnIndex(Media._ID));
                mVideoAlbum.mCount = cursorVideo.getCount();
                mVideoAlbum.mImageList.add(new VideoMedia(id, picPath));
            }
        }finally {
            if (cursorVideo != null) {
                cursorVideo.close();
    ...
    private void getAlbumList(@NonNull final IAlbumTaskCallback callback) {
        ...
        if (tmpList.size() > 0 && tmpList.get(0) != null) {
            ...
            if(mPickerConfig.isNeedVideo()&&mVideoAlbum.mCount>0){
                tmpList.add(1,mVideoAlbum);
            }
            //========yqy changed 2017-09-29===================
        }
